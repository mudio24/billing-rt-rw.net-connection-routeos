...
class MikrotikAPI {
  constructor(options) {
    ...
    this.timeout = (options.timeout || 4) * 1000;
    this.socket = null;
    this.connected = false;
    this._buffer = Buffer.alloc(0);
    // Write lock — only 1 command at a time on the socket
  ...
  async write(command, params = {}) {
    ...
    // Queue this command — wait for any previous command to finish first
    let release;
    ...
    try {
      return await this._executeWrite(command, params);
    } finally {
      release();
    }
  }
  ...
  /**
   * Internal: actually send command and read response (only called under lock)
   */
  async _executeWrite(command, params) {
    const words = [command];
