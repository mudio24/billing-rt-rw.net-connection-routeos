...
class MikrotikServiceManager extends EventEmitter {
  ...
  isConnected(routerId) {
    return this.connections.has(Number(routerId));
  }
  ...
  // Traffic monitoring cache for delta calculation
  _trafficCache = new Map();

  async getInterfaceTraffic(id, interfaceName) {
    const client = this.getClient(id);
    if (!client) throw new Error('Not connected');
    
    // Fetch all interfaces and filter in JS (RouterOS 6.x doesn't support ?name= well)
    const allIfaces = await client.write('/interface/print');
    const iface = allIfaces.find(i => i.name === interfaceName);
    ...
    const cacheKey = `${id}_${interfaceName}`;
    const prev = this._trafficCache.get(cacheKey);
    
    this._trafficCache.set(cacheKey, { rxBytes, txBytes, time: now });
    ...
    // Handle counter reset (router reboot)
    const rxDelta = rxBytes >= prev.rxBytes ? rxBytes - prev.rxBytes : rxBytes;
    const txDelta = txBytes >= prev.txBytes ? txBytes - prev.txBytes : txBytes;
    
    const rxBps = Math.round((rxDelta * 8) / timeDiffSec);
    const txBps = Math.round((txDelta * 8) / timeDiffSec);
    
    return [{ 'rx-bits-per-second': String(rxBps), 'tx-bits-per-second': String(txBps) }];
  }
